            ...
            <div className='flex w-[200%] animate-marqueeBottom'>
                {duplicatedImages.map((images, index) => (
                    <div key={`bottom-${index}`} className='flex-shrink-0'>
                        ...
                        {/* <div className='inset-0 flex items-end bg-gradient-to-t from-black/80 to-transparent p-4 opacity -0 transition-opacity duration-300 group-hover:opacity-100'> */}<div>
                            <span className='text-white font-medium'>{images.caption}</span>
                        </div>
                    </div>
                ))}
